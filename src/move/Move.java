...
//
//    public ArrayList<String> getAllValidMoves(char turn){
...
//                moves.addAll(moveGenerator.generateMove(file, rank));
//            }
//        }
//        return moves;
//    }
    ...
    public void makeMove(String move){
        ...
        if(move.equalsIgnoreCase(Constants.KING_SIDE_CASTLING) || move.equalsIgnoreCase(Constants.QUEEN_SIDE_CASTLING)){
            cb.captured = Constants.EMPTY_SQUARE;
            ...
            cb.prevEnpassant = cb.fenParts[10];
            cb.fenParts[10] = "-";
            ...
            cb.captured = cb.board[rank][destFile];
            ...
            cb.captured = cb.board[destRank][destFile];
            ...
            if(Character.toUpperCase(cb.board[destRank][destFile]) == Constants.WHITE_PAWN){
                // en-passant
                if(move.substring(2,4).equals(cb.fenParts[10])){
                    cb.captured = cb.board[locRank][destFile];
    ...
    public void unMakeMove(String move){
        ...
        if(move.equalsIgnoreCase(Constants.KING_SIDE_CASTLING) || move.equalsIgnoreCase(Constants.QUEEN_SIDE_CASTLING)){
            ...
            cb.fenParts[9] = cb.prevCastle;
            cb.fenParts[rank] = FenUtils.getRank(cb.board[rank]);
            cb.fenParts[8] = Character.toString(cb.turn);
            cb.fenParts[10] = cb.prevEnpassant;
        }else if(move.charAt(1) == move.charAt(3)){
            int rank = 8-Integer.parseInt(String.valueOf(move.charAt(1)));
            int destFile = Constants.FILES.indexOf(move.charAt(0));
            int locFile = Constants.FILES.indexOf(move.charAt(2));
            if(cb.board[rank][locFile] == Constants.WHITE_KING){
                cb.whiteKingPosition[0] = destFile;
                cb.whiteKingPosition[1] = rank;
            }else if(cb.board[rank][locFile] == Constants.BLACK_KING){
            ...
            cb.board[rank][destFile] = cb.board[rank][locFile];
            cb.board[rank][locFile] = cb.captured;
            cb.pieceLocations.add(destFile + rank * 8);
            if(cb.captured == Constants.EMPTY_SQUARE) {
                cb.pieceLocations.remove((Object) (locFile + rank * 8));
            }
            ...
            int locRank = 8-Integer.parseInt(String.valueOf(move.charAt(3)));
            int destRank = 8-Integer.parseInt(String.valueOf(move.charAt(1)));
            int locFile = Constants.FILES.indexOf(move.charAt(2));
            int destFile = Constants.FILES.indexOf(move.charAt(0));
            if(cb.board[locRank][locFile] == Constants.WHITE_KING){
                cb.whiteKingPosition[0] = destFile;
            ...
            cb.board[locRank][locFile] = cb.captured;
            cb.pieceLocations.add(destFile + destRank * 8);
            if(cb.captured == Constants.EMPTY_SQUARE) {
                cb.pieceLocations.remove((Object) (locFile + locRank * 8));
            }
            ...
            if(Character.toUpperCase(cb.board[destRank][destFile]) == Constants.WHITE_PAWN){
                // en-passant
                if(move.substring(2,4).equals(cb.prevEnpassant)){
                    cb.board[locRank][locFile] = Constants.EMPTY_SQUARE;
                    cb.board[destRank][locFile] = cb.captured;
                    cb.pieceLocations.remove((Object)(locFile + locRank * 8));
                    cb.pieceLocations.add(locFile + destRank*8);
                }else if (locRank == 0 || locRank == 7){
                    // pawn-promotion
                    if(Character.isUpperCase(cb.board[destRank][destFile])){
                        cb.board[destRank][destFile] = Constants.WHITE_PAWN;
                    }else{
                        cb.board[destRank][destFile] = Constants.BLACK_PAWN;
                    }
                }
            }
