...
public class MoveGenerator {
    ...
    public MoveGenerator(ChessBoard cb){
        this.cb = cb;
        moves = new ArrayList<>();
    }
    ...
    public ArrayList<String> king(final int file,final int rank){
        for(int[] direction:Constants.ALL_DIRECTIONS){
            ...
            if(cb.isSquareSafe(newFile,newRank)){
        ...
        if(kingSide){
            for(int i=file;kingSide&&i<file+3;i++){
                kingSide = cb.isSquareSafe(i,rank);
            }
            if(kingSide){
                moves.add(Constants.KING_SIDE_CASTLING);
            }
        }
        if(queenSide){
            for(int i=file;queenSide&&i>file-3;i--){
                queenSide = cb.isSquareSafe(i,rank);
            }
            if(queenSide){
                moves.add(Constants.QUEEN_SIDE_CASTLING);
            }
        }
        return moves;
    }
